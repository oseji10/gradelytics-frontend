...
export default function AffectiveDomainEntry() {
  ...
  const fetchDomains = async () => {
    try {
      ...
      const data = res.data?.affective || [];
      setDomains(Array.isArray(data) ? data : []);
    } catch (err) {
      toast.error("Failed to load affective domains");
      setDomains([]);
    } finally {
      ...
      <div className="space-y-8 px-4 py-8 md:px-8 lg:px-12">
        ...
        <Card>
          <CardHeader className="pb-4">
            ...
            <CardDescription>Choose class to enter affective ratings</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-4">
            <CardTitle>
              {selectedClassId
                ? `Affective Domains – ${classes.find((c) => c.classId === Number(selectedClassId))?.className}`
          ...
          <CardContent>
            {loadingStudents || loadingDomains ? (
              ...
              <div className="py-20 text-center text-amber-600">
                No affective domains have been defined yet.
                <br />
