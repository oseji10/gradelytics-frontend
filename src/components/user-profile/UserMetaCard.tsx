...
export default function UserMetaCard() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setFormData({
          firstName: response.data.firstName || "Musharof",
          lastName: response.data.lastName || "Chowdhury",
          email: response.data.email || "randomuser@pimjo.com",
          phone: response.data.phone || "+09 363 398 46",
          bio: response.data.bio || "Team Manager",
          facebook: response.data.facebook || "https://www.facebook.com/PimjoHQ",
          twitter: response.data.twitter || "https://x.com/PimjoHQ",
  ...
  const handleSave = async () => {
    setUpdating(true);
    try {
      await api.patch("/profile", formData); // Adjust endpoint as needed
          ...
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            {/* <div className="w-20 h-20 overflow-hidden border border-gray-200 rounded-full dark:border-gray-800">
              <Image
                width={80}
                height={80}
                src={userData?.profileImage || "/images/user/owner.jpg"}
                alt="user"
              />
            </div> */}
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form className="flex flex-col" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              <Button size="sm" variant="outline" onClick={closeModal} disabled={updating}>
                Close
              </Button>
