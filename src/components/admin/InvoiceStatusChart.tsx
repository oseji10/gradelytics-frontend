...
export default function InvoiceStatusChart({
  ...
  useEffect(() => {
    setLoading(true);
    api
      .get("/admin/invoice-status-breakdown", {
  ...
  const stackedData = isStacked
    ? [
        data.reduce(
          (acc: Record<string, number | string>, item: { status: string; count: number }) => {
    ...
    <div className={cardClass}>
      <div className="flex items-start justify-between gap-2 mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">Distribution of open, paid, and overdue invoices</p>
        </div>
        <span className="rounded-full bg-[#1F6F43]/10 px-2 py-0.5 text-[11px] font-semibold text-[#1F6F43]">
          {rangeLabel}
        </span>
      ...
      <ResponsiveContainer width="100%" height={280}>
        {isStacked ? (
          <BarChart data={stackedData} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
            ...
            />
            {data.map((entry: { status: string }, index: number) => (
              <Bar
