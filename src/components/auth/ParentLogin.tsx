// app/result-checker/login/page.tsx
...
import { AlertCircle, Lock, Phone, Loader2 } from "lucide-react";
...
interface School {
  schoolId: number;
  schoolName: string;
  // add more fields if needed (shortName, logo, etc.)
}
...
export default function LoginPage() {
    ...
    const fetchSchools = async () => {
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/schools`, {
          ...
          // credentials: "include" // add if needed for public list
        });
        ...
        // e.g. data.schools, data.data, etc.
        const schoolList = Array.isArray(data) ? data : data.schools || data.data || [];
        setSchools(schoolList);

        // Optional: auto-select first school if only one exists
        if (schoolList.length === 1) {
          setSelectedSchoolId(schoolList[0].id.toString());
        }
      } catch (err: any) {
        console.error(err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!phoneNumber || !pin || !selectedSchoolId) {
    ...
    try {
      const loginResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/parent/verify`,
        {
          method: "POST",
          headers: {
      ...
      // On success → redirect to dashboard (no query params needed)
      router.push("/result-checker/dashboard");
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="mt-auto mb-12">
              <h1 className="text-4xl xl:text-5xl font-bold leading-tight mb-6">
                Access your child's results securely
              </h1>
              <p className="text-lg xl:text-xl text-green-100/90 max-w-md">
                Select your school, then enter your phone number and access PIN to view performance, attendance, and insights.
              </p>
            </div>
        ...
        <div className="flex items-center justify-center p-6 sm:p-10 lg:p-12 bg-white">
          <div className="w-full max-w-md space-y-8">
            ...
            <div className="lg:hidden text-center mb-8">
              ...
              <p className="text-gray-600 mt-2">Sign in to view results</p>
            </div>
            ...
            <Card className="border-none shadow-none lg:shadow-lg lg:border">
              <CardHeader className="space-y-1 pb-6">
                ...
                <CardDescription className="text-center lg:text-left">
                  Select your school and enter your credentials
                </CardDescription>
              </CardHeader>
              ...
              <CardFooter className="flex flex-col space-y-4 pt-2 text-center text-sm text-gray-600">
                <p className="text-gray-500">
                  Contact school admin if you don’t have an access PIN yet.
                </p>
              </CardFooter>
            </Card>
          </div>
